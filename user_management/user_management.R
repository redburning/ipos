  ...
  observeEvent(input$useradd_confirm, {
    tryCatch({
      if (input$user == "") {
        ...
        addusersql <- paste0("INSERT INTO users (user, password, password_hash, permissions, lock) 
                           VALUES ", "('", input$user, "', '", input$password, "', '", sodium::password_store(input$password), "', '", input$permissions, "', ' 0')")
        cat(addusersql)
