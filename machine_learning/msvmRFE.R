...
# Feature selection with Multiple SVM Recursive Feature Elimination (RFE) algorithm
    ...
    while(length(i.surviving) > 0) {
        if(k > 1) {
            ...
            w = lapply(folds, getWeights, X[, c(1, 1+i.surviving)], kernel, cost)
            w = do.call(rbind, w)
            ...
            w = getWeights(NULL, X[, c(1, 1+i.surviving)], kernel = kernel, cost = cost)
            c = w * w
        }
...
getWeights <- function(test.fold, X, kernel, cost) {
# Fit a linear SVM model and obtain feature weights
    train.data = X
    if(!is.null(test.fold)) train.data = X[-test.fold, ]
    
    svmModel = svm(train.data[, -1], train.data[, 1], cost=cost, cachesize=500, scale=F, type="C-classification", kernel=kernel)
