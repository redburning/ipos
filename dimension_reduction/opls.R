    ...
    tryCatch({
      if (is.null(dataset)) {
          ...
          output$vipbubble_plot <- renderPlot({
            ...
            for (i in 2:(ncol(selected_data_group1) - 1)) {
              if (input$opls_vipbubbleplot_pvalue_calc_method == 'T-test') {
                p = t.test(selected_data_group1[, i], selected_data_group2[, i])[["p.value"]]
              } else {
                p = wilcox.test(selected_data_group1[, i], selected_data_group2[, i])[["p.value"]]
              }
        ...
        output$bio_plot <- renderPlot({
          ...
          c <- ncol(data_matrix)
          r <- nrow(data_matrix)
          # biodata <- data.frame(matrix(ncol = 2, nrow = c + r)) %>% 
