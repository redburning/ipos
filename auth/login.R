...
loginUI <- function(id,
                    ...
                    user_placeholder = "Please input username",
                    pass_title = "Password",
                    pass_placeholder = "Please input password",
                    login_title = "Log in",
  ...
  shinyjs::hidden(
    shiny::div(
      ...
      shiny::wellPanel(
        ...
        fluidRow(
          column(width = 12,
                 shiny::textInput(ns("user_name"), label = user_title, placeholder = user_placeholder),
                 tags$style(type="text/css", "#login-user_name { border-radius: 5px; height: 40px}")
          )
        ),
        
        fluidRow(
          column(width = 12,
                 shiny::passwordInput(ns("password"), label = pass_title, placeholder = pass_placeholder),
                 tags$style(type="text/css", "#login-password { border-radius: 5px; height: 40px}")
          )
        ),
        
        shiny::div(
          style = "text-align: center;",
          shiny::actionButton(ns("button"), login_title, class="login-button")
        ),
        additional_ui,
        shinyjs::hidden(
          shiny::div(
...
loginServer <- function(id, 
  ...
  try_class_lc <- try(class(lock_col), silent = TRUE)
  if (try_class_lc == "character") {
    lock_col <- rlang::sym(lock_col)
  }
  
  try_class_ec <- try(class(expiredtime_col), silent = TRUE)
  if (try_class_ec == "character") {
    expiredtime_col <- rlang::sym(expiredtime_col)
  }
    ...
    function(input, output, session) {
      ...
      shiny::observeEvent(input$button, {
        ...
        if (length(row_username)) {
          ...
          # 获取账户锁定状态
          row_lock <- dplyr::filter(data, dplyr::row_number() == row_username)
          row_lock <- dplyr::pull(row_lock, {{lock_col}})
          lock <- ifelse(row_lock == "1", TRUE, FALSE)
          # 获取账户有效期限
          expired_time_df <- dplyr::filter(data, dplyr::row_number() == row_username)
          expired_time <- expired_time_df[["expired_time"]]
          expired <- ifelse(expired_time != "" && !is.null(expired_time) && format(Sys.time(), "%Y-%m-%d") > expired_time, TRUE, FALSE)
          if (sodium_hashed) {
        ...
        if (length(row_username) == 1 && password_match && lock == FALSE && expired == FALSE) {
          ...
          shinyjs::toggle(id = "lockmessage", anim = TRUE, time = 1, animType = "fade")
          shinyjs::delay(5000, shinyjs::toggle(id = "lockmessage", anim = TRUE, time = 1, animType = "fade"))
        } else if (expired) {
          shinyjs::toggle(id = "expiredmessage", anim = TRUE, time = 1, animType = "fade")
          shinyjs::delay(5000, shinyjs::toggle(id = "expiredmessage", anim = TRUE, time = 1, animType = "fade"))
