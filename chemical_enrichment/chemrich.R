run_chemrich_basic <- function(data) {
  ndf <- data
  ndf$edirection <- "up"
  ndf$efs <- 1
  ...
  clustdf <- clusterdf[which(clusterdf$pvalues!=1.0),]
  ...
  p2 <- p2 + geom_point(aes(size=altratio, color=upratio)) +
    ...
    scale_colour_gradient(low = "#2861AD", high = "#DA1716", space = "Lab",guide = "colourbar", limits=c(0,1))+
    scale_size(range = c(5, 30)) +
    scale_y_continuous("-log (pvalue)",limits = c(0, max(-log(clustdf.alt.impact$pvalues))+4  )) +
    scale_x_continuous(" Chemical Class Set ") +
    theme_bw() +
    #labs(title = "ChemRICH cluster impact plot") +
    geom_text_repel(aes(label = name), color = "gray20",family="Arial",data=subset(clustdf.alt.impact, csize>1 & pvalues<0.05),force = 0, vjust = 0.5, nudge_y = 2)+
  ...
  #   ph_with(dml(ggobj = p2), location = ph_location(type = "body",width=10, height=6.18,left = 0, top = 0)) %>%
  #   print(target = paste0("chemrich_class_impact_plot.pptx")) %>%
  #   invisible()
  
  # ggsave(paste0("chemrich_class_impact_plot.eps"), p2, height = 6.18, width = 10, dpi=600)
  ...
  clustdf.e$upratio <- signif(clustdf.e$upratio, digits = 3)
  clustdf.e$altratio <- signif(clustdf.e$altratio, digits = 3)
  ...
  ndf$pvalue <- signif(ndf$pvalue, digits = 4)
  ndf$efs <- signif(ndf$efs, digits = 4)
  ndf$FDR <- signif(  p.adjust(ndf$pvalue), digits = 4)
  ...
  p2 <- p2 + geom_point(aes(x = xlogp, y = -log(pvalues), size = altratio, color = upratio)) +
    ...
    scale_color_gradient(low = "#2861AD", high = "#DA1716", space = "Lab", guide = "colourbar", limits = c(0, 1)) +
    scale_size(range = c(5, 20)) +
    scale_y_continuous("-log (pvalue)", limits = c(0, max(-log(clustdf.alt.impact$pvalues)) + 5)) +
    scale_x_continuous("Chemical Class Set ") +
    theme_bw() +
    #labs(title = "ChemRICH cluster impact plot") +
    geom_text(aes(x = xlogp, y = -log(pvalues), label = name), color = "gray20", data = subset(clustdf.alt.impact, csize > 2 & pvalues < 0.05))+
    theme(
