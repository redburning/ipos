...
ora_result_lollipopplot_width <- 600
ora_result_lollipopplot_height <- 600
ora_result_dotplot_width <- 800
ora_result_dotplot_height <- 600


oraEnrichmentUI <- function(id) {
  ns <- NS(id)
  useToastr()
  useShinyjs()
  fluidPage(
    fluidRow(
      tags$div(
        ...
        fluidRow(
               ...
               <div id="detailDialog" class="dialog">
                    	...
                    	<tbody>
                    	  <tr>
                    		...
                    		<tr class="content">
                    			<td><div id="setName"></div></td>
                    			<td><div id="candidates"></div></td>
                    			<td><div id="desc" style="white-space:break-spaces;"></div></td>
                    		</tr>
                    	</tbody>
                    </table>
            			</div>
          ...
          column(width = 12,
                 box(id = "box-ora", title = "ORA(Over-Representation Analysis)", collapsible = TRUE, collapsed = TRUE, width = 12,
                     solidHeader = TRUE,
                                               ...
                                               fluidRow(
                                                 ...
                                                 column(width = 9,
                                                        withSpinner(plotOutput(outputId = ns("ora_result_lollipopplot"), 
                                                                               width = paste0(ora_result_lollipopplot_width, "px"), 
                                                                               height = "auto")
                                                                    )
                                                        ),
                                                 column(width = 3,
                                                        ...
                                                        tags$div(
                                                          ...
                                                          box(title = "基础设置", collapsible = TRUE, collapsed = TRUE, width = 12,
                                                              ...
                                                              fluidRow(
                                                                column(width = 6, 
                                                                       numericInput(inputId = ns("ora_result_lollipopplot_width"),
                                                                                    label = "宽度",
                                                                                    value = ora_result_lollipopplot_width)
                                                                       ),
                                                                column(width = 6, 
                                                                       numericInput(inputId = ns("ora_result_lollipopplot_height"),
                                                                                    label = "高度",
                                                                                    value = ora_result_lollipopplot_height)
                                                                       )
                                                                )
                                                              ),
                                                          ),
                                               ...
                                               fluidRow(
                                                 ...
                                                 column(width = 9,
                                                        withSpinner(plotOutput(outputId = ns("ora_result_dotplot"),
                                                                               width = paste0(ora_result_dotplot_width, "px"),
                                                                               height = "auto")
                                                                    )
                                                        ),
                                                 column(width = 3,
                                                        ...
                                                        tags$div(
                                                          ...
                                                          box(title = "基础设置", collapsible = TRUE, collapsed = TRUE, width = 12,
                                                              ...
                                                              fluidRow(
                                                                column(width = 6,
                                                                       numericInput(inputId = ns("ora_result_dotplot_width"),
                                                                                    label = "宽度",
                                                                                    value = ora_result_dotplot_width)
                                                                ),
                                                                column(width = 6,
                                                                       numericInput(inputId = ns("ora_result_dotplot_height"),
                                                                                    label = "高度",
                                                                                    value = ora_result_dotplot_height)
                                                                )
                                                              )
                                                          ),
                                                        ),
...
oraEnrichmentServer <- function(input, output, session) {
  ns <- session$ns
  querylist <- NULL
  bglist <- NULL
  oraResult <- NULL
  ...
  createDetailButton <- function(data) {
    ...
    for (i in 1:nrow(data)) {
      ...
      btnHtml = sprintf('<button class="dataTables_button_details" onclick="(function() {
                ...
                document.getElementById(\'setName\').innerHTML=`%s`;
  ...
  observeEvent(input$execute, {
    ...
    if (!is.null(querylist)) {
      ...
      if (is.null(bglist)) {
        enrichResult <- enricher(querylist[[1]], 
                                 TERM2GENE = base, 
                                 minGSSize = minGSSize, 
                                 maxGSSize = maxGSSize, 
      ...
      if (!is.null(oraResult) && nrow(oraResult) > 0) {
        ...
        output$ora_result_lollipopplot <- renderPlot({
          ...
          if (nrow(plotData) > 0) {
            ...
            ora_result_lollipopplot_height <<- nrow(plotData) * 20 + 40
            updateNumericInput(inputId = "ora_result_lollipopplot_height", value = ora_result_lollipopplot_height)
          ...
          ora_result_lollipopplot_height <<- input$ora_result_lollipopplot_height
          return(ora_result_lollipopplot_height)
        }, width = function() {
          ora_result_lollipopplot_width <<- input$ora_result_lollipopplot_width
          return(ora_result_lollipopplot_width)
        })
        ...
        output$ora_result_dotplot <- renderPlot({
          ...
          if (nrow(plotData) > 0) {
            # 自适应更新ora-lollipop-plot的高度
            ora_result_dotplot_height <<- nrow(plotData) * 20 + 40
            updateNumericInput(inputId = "ora_result_dotplot_height", value = ora_result_dotplot_height)
          ...
          ora_result_dotplot_height <<- input$ora_result_dotplot_height
          return(ora_result_dotplot_height)
        }, width = function() {
          ora_result_dotplot_width <<- input$ora_result_dotplot_width
          return(ora_result_dotplot_width)
        })
      } else {
  ...
  output$download_ora_result_lollipopplot_ok <- downloadHandler(
    ...
    content = function(file) {
      ggsave(filename = file, 
             ...
             width = ora_result_lollipopplot_width / plot_size_fold,
             height = ora_result_lollipopplot_height / plot_size_fold,
             units = "mm")
      removeModal()
    }
  )
  ...
  output$download_ora_result_dotplot_ok <- downloadHandler(
    ...
    content = function(file) {
      ggsave(filename = file, 
             ...
             width = ora_result_dotplot_width / plot_size_fold,
             height = ora_result_dotplot_height / plot_size_fold,
             units = "mm")
      removeModal()
    }
  )
  ...
  callModule(helpServer, "help_analysis", title = "ORA", size = "l", file = "help/BiologyEnrich.md")
}